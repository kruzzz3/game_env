package ch.webk.light.box2dLight;
...
public abstract class PositionalLight extends Light {
	...
	public PositionalLight(RayHandler rayHandler, int rays, Color color, float distance, float x, float y, float directionDegree) {
		...
		start.x = x * Constants.WORLD_TO_SCREEN;
		start.y = y * Constants.WORLD_TO_SCREEN;
	...
	public void attachToBody(Body body, float offsetX, float offSetY, float degrees) {
		this.body = body;
		bodyOffsetX = offsetX * Constants.WORLD_TO_SCREEN;
		bodyOffsetY = offSetY * Constants.WORLD_TO_SCREEN;
	...
	public void setPosition(Vector2 position) {
		start.x = position.x * Constants.WORLD_TO_SCREEN;
		start.y = position.y * Constants.WORLD_TO_SCREEN;
		if (staticLight) dirty = true;
	}
	...
	protected void updateBody() {
		...
		start.x = (vec.x * Constants.WORLD_TO_SCREEN) + dX;
		start.y = (vec.y * Constants.WORLD_TO_SCREEN) + dY;
	...
	protected void updateMesh() {
		for (int i = 0; i < rayNum; i++) {
			...
			if (rayHandler.world != null && !xray) {
                Vector2 startTemp = new Vector2(start.x / Constants.WORLD_TO_SCREEN, start.y / Constants.WORLD_TO_SCREEN);
                Vector2 endTemp = new Vector2(tmpEnd.x / Constants.WORLD_TO_SCREEN, tmpEnd.y / Constants.WORLD_TO_SCREEN);
