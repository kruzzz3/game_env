...
public abstract class Light implements Disposable {
	...
	protected float softShadowLength = 0.1f;
	...
	public void setSoftnessLength(float softShadowLength) {
		this.softShadowLength = softShadowLength * Constants.WORLD_TO_SCREEN;
		if (staticLight) dirty = true;
	}
	...
	final RayCastCallback ray = new RayCastCallback() {
		...
		final public float reportRayFixture(Fixture fixture, Vector2 point,
			...
			mx[m_index] = point.x * Constants.WORLD_TO_SCREEN;
			my[m_index] = point.y * Constants.WORLD_TO_SCREEN;
			f[m_index] = fraction;
			return fraction;
		}
	};
